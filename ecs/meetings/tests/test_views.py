...
class ViewTestCase(EcsTestCase):
    ...
    def test_timetable(self):
        ...
        timetable_url = reverse('meetings.timetable_editor', kwargs={'meeting_pk': meeting.pk})
        response = self.client.get(timetable_url)
        self.assertEqual(response.status_code, 200)
        
        e0 = meeting.add_entry(duration=timedelta(seconds=42))
        response = self.client.post(reverse('meetings.update_timetable_entry', kwargs={'meeting_pk': meeting.pk, 'entry_pk': e0.pk}), {
            'duration': '2:00:00',
        })
        self.assertEqual(response.status_code, 302)
        ...
        response = self.client.get(reverse('meetings.move_timetable_entry', kwargs={'meeting_pk': meeting.pk}) + '?from_index=0&to_index=1')
        self.assertEqual(response.status_code, 302)
        self.assertEqual(list(self.refetch(meeting)), [e1, e0])
        
        response = self.client.get(reverse('meetings.remove_timetable_entry', kwargs={'meeting_pk': meeting.pk, 'entry_pk': e0.pk}))
        self.assertEqual(response.status_code, 302)
    ...
    def test_meeting_assistant(self):
        ...
        response = self.client.get(reverse('meetings.meeting_assistant', kwargs={'meeting_pk': meeting.pk}))
        self.assertEqual(response.status_code, 200)
        
        response = self.client.get(reverse('meetings.meeting_assistant_start', kwargs={'meeting_pk': meeting.pk}))
        self.assertEqual(response.status_code, 302)
        ...
        response = self.client.get(reverse('meetings.meeting_assistant_top', kwargs={'meeting_pk': meeting.pk, 'top_pk': e0.pk}))
        self.assertEqual(response.status_code, 200)
        
        response = self.client.post(reverse('meetings.meeting_assistant_top', kwargs={'meeting_pk': meeting.pk, 'top_pk': e0.pk}), {
            'close_top': 'on',
            'result': '1',
        })
        ...
        response = self.client.get(reverse('meetings.meeting_assistant_stop', kwargs={'meeting_pk': meeting.pk}))
        self.assertEqual(response.status_code, 404)

        response = self.client.post(reverse('meetings.meeting_assistant_top', kwargs={'meeting_pk': meeting.pk, 'top_pk': e1.pk}), {
            'close_top': 'on',
            'result': '2',
        })
        self.assertTrue(response.status_code, 302)
    ...
    def test_meeting_assistant_quickjump(self):
        ...
        quickjump_url = reverse('meetings.meeting_assistant_quickjump', kwargs={'meeting_pk': meeting.pk})
        ...
        self.assertEqual(urlsplit(last_url).path, reverse('meetings.meeting_assistant_top', kwargs={'meeting_pk': meeting.pk, 'top_pk': e0.pk}))
