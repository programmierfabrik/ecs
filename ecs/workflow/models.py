...
class NodeType(models.Model):
    ...
    content_type = models.ForeignKey(ContentType, null=True, related_name='workflow_node_types', on_delete=models.PROTECT)
    implementation = models.CharField(max_length=200)
    data_type = models.ForeignKey(ContentType, null=True, on_delete=models.PROTECT)
...
class Node(models.Model):
    ...
    graph = models.ForeignKey(Graph, related_name='nodes', on_delete=models.PROTECT)
    node_type = models.ForeignKey(NodeType, on_delete=models.PROTECT)
    data_id = models.PositiveIntegerField(null=True)
    data_ct = models.ForeignKey(ContentType, null=True, on_delete=models.PROTECT)
...
class Edge(models.Model):
    from_node = models.ForeignKey(Node, related_name='edges', null=True, on_delete=models.PROTECT)
    to_node = models.ForeignKey(Node, related_name='incoming_edges', null=True, on_delete=models.PROTECT)
    deadline = models.BooleanField(default=False)
    guard = models.ForeignKey(Guard, related_name='nodes', null=True, on_delete=models.PROTECT)
...
class Workflow(models.Model):
    ...
    graph = models.ForeignKey(Graph, related_name='workflows', on_delete=models.PROTECT)
    is_finished = models.BooleanField(default=False)
    parent = models.ForeignKey('workflow.Token', null=True, related_name='parent_workflow', on_delete=models.PROTECT)
...
class Token(models.Model):
    workflow = models.ForeignKey(Workflow, related_name='tokens', on_delete=models.PROTECT)
    node = models.ForeignKey(Node, related_name='tokens', on_delete=models.PROTECT)
    trail = models.ManyToManyField('self', related_name='future', symmetrical=False)
    source = models.ForeignKey(Node, related_name='sent_tokens', null=True, on_delete=models.PROTECT) # denormalized: can be derived from trail
    deadline = models.DateTimeField(null=True)
    locked = models.BooleanField(default=False)
    repeated = models.BooleanField(default=False)
    created_at = models.DateTimeField(auto_now_add=True)
    consumed_at = models.DateTimeField(null=True, blank=True, default=None)
    consumed_by = models.ForeignKey(User, null=True, blank=True, on_delete=models.PROTECT)
